...
class Notifications(models.Model):
    ...
    def get_valid_notifcations():
        """Return valid notifications."""
        data = []  # holds data to be returned
        today = date.today()
        valid_notifications = Notifications.objects.filter(
            validity__gte=(today - F("date_submitted"))
        )
            ...
            for valid_notification in valid_notifications:
                ...
                if valid_notification.upload:
                    notification["upload_url"] = (
                        "http://127.0.0.1:8000" + valid_notification.upload.url
                    )
                else:
                    notification["upload_url"] = ""
